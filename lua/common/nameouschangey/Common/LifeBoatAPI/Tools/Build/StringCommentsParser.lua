...
require("sw-micro-project.lua.common.nameouschangey.Common.LifeBoatAPI.Tools.Build.StringReplacer")
require("sw-micro-project.lua.common.nameouschangey.Common.LifeBoatAPI.Tools.Build.CommentReplacer")
  ...
  removeStringsAndComments = function(this, text, commentException)
    ...
    for i = 1, #text, 1 do
      ...
      elseif not isInComment and isInQuote then
        if
          (not cUnescapedSlash and c == quoteType and quoteType ~= "[")
          or (quoteType == "[" and cLast == "]" and c == "]")
        then
          currentString:add(c)
          isInQuote = false
